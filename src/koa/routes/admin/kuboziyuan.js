...
router
    .get('/', async (ctx) => {
        ...
        if (typeid == 0) {
            url = 'http://www.kubozy.net/?m=vod-index-pg-' + (page) + '.html';
        } else {
            url = 'http://www.kubozy.net/?m=vod-type-id-' + typeid + '-pg-' + (page) + '.html';
        }
        let datalist = [];
        ...
        await ctx.render('admin/kuboziyuan/index', {
            list: datalist,
            total: total[1],
            curpage: (page - 1),
            typeid: typeid
    ...
    .get('/info', async (ctx) => {
        let html = await Html.getHtml('http://www.kubozy.net/' + ctx.request.query.url);
        ...
        tempTr("div#2 > ul > li").each(function(i, e) {
        ...
        // console.log(info);
        await ctx.render('admin/kuboziyuan/info', {
            info: info,
        });
    })
    .all('/seach', async (ctx) => {
        ...
        var typeid = 0;
        let page = 1;
        url = 'http://www.kubozy.net/index.php?m=vod-search';
        let datalist = [];
        let html;
        if (wd == "" || wd == null) {
